...
async def output_log(message: types.Message):
    ...
    print(1)
    orders = Orders()
    await bot.send_message(message.from_user.id, 'Список платежей:')
    if id_check == ID_MASTER:
        data = []
        ...
        df = pd.DataFrame(data, columns=['ID заказа', 'ID пользователя', 'имя пользователя', 'полное имя', 'дата создания', 'дата подтверждения',
                                         'статус', 'артикул', 'цена'])
        buffer = io.BytesIO()
...
def handlers_register_manage(dp: Dispatcher):
    ...
    dp.register_message_handler(output_log, Text(equals='Платежи', ignore_case=True))
